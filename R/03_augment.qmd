...
write_tsv(
  data_treatment,
  here("data/03_dat_all_augment.tsv")
)
```
...
below_q_value <- c("ASCL1_NM_004316", "AVP_NM_000490", "AZGP1_NM_001185", "BEX1_NM_018476", "C11orf53_NM_198498", "CALML3_NM_005185", "CALML5_NM_017422",   "CD164_NM_001142404", "CDKN2A_NM_058195",      "CEACAM5_NM_004363",     "CHGA_NM_001275",        "CSAG1_NM_001102576",
...
which_clust_high_low <- avg_expression_clust |>
  group_by(transcript_id) |>
  summarise(avg_max = max(c(cluster_1, cluster_2)),
...
high_low_clustering <- clust_data_long |>
  ...
  left_join(which_clust_high_low,
            by = 'transcript_id') |>
  mutate(high_low_clust = case_when(cluster_int == 1 & cluster_1_cat == 1 ~1,
...
gene_data <- data.frame(
  sample_id = rep(c("Sample1", "Sample2", "Sample3",
                ...
                "Sample28", "Sample29", "Sample30"), each = 1),
...
fisher_results <- gene_data_long
  group_by(transcript_id) |>
  summarise(
    p_value = fisher.test(table(high_low_clust, sample_id))$p.value
  )
...
fisher_results %>%
  ...
  labs(title = "Fisher's Exact Test for transcript Expression",
       x = "transcript_id",
       y = "-log10(p-value)")
...
select_transcript_meta |>
  write_tsv(here("data/03_dat_selected_transcript_augment.tsv"))
```
